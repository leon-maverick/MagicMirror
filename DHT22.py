...
if __name__ == "__main__":
   ...
   s = DHT22.sensor(pi, 3, LED=16, power=8)
   s.trigger()
   ...
   room_rh = int(round(s.humidity()))
   room_temp = s.temperature()
   filename = "Room_Temperature.txt"
   target = open(filename,'w')
   target.write(str (room_temp))

   filename = "Room_Humidity.txt"
   target = open(filename,'w')
   target.write(str (room_rh))
